...
function parseArgs() {
  ...
  if (!itemArg) {
    console.error('Usage: node scripts/getGalleryImageUrls.js item:ItemName [tabbed:VariantAlt] [gallery:GalleryAlt or [a, b, c]] [altsuffix:AltSuffix]');
    process.exit(1);
  }
  ...
  if (gallery && gallery.startsWith('[') && gallery.endsWith(']')) {
    gallery = gallery.slice(1, -1).split(',').map(s => s.trim().replace(/^"|"$/g, '').replace(/^'|'$/g, ''));
  }
  return {
...
async function updateItemInFirestore(itemName, imageUrl, wikiUrl) {
  ...
  for (const doc of snapshot.docs) {
    if (DRY_RUN) {
      console.log(`[DRY RUN] Would update Firestore item '${itemName}' with image: ${imageUrl}`);
    } else {
...
async function main() {
  ...
  if (Array.isArray(gallery) && gallery.length > 0) {
    for (const galleryItem of gallery) {
      ...
      if (imageUrl) {
        console.log(`Image URL for '${itemName}' (gallery: ${galleryItem}${altsuffix ? ' ' + altsuffix : ''}): ${imageUrl}`);
        console.log(`Wiki page: ${finalUrl}`);
        await updateItemInFirestore(`${galleryItem} ${altsuffix || ''}`.trim(), imageUrl, finalUrl);
      } else {
        console.log(`No image found for '${itemName}' (gallery: ${galleryItem}${altsuffix ? ' ' + altsuffix : ''}) on ${finalUrl}`);
      }
    }
  } else {
    let imageUrl;
    ...
    if (imageUrl) {
      console.log(`Image URL for '${itemName}'${tabbed ? ` (tabbed: ${tabbed})` : ''}${gallery ? ` (gallery: ${gallery})` : ''}: ${imageUrl}`);
      console.log(`Wiki page: ${finalUrl}`);
      await updateItemInFirestore(itemName, imageUrl, finalUrl);
    } else {
      console.log(`No image found for '${itemName}'${tabbed ? ` (tabbed: ${tabbed})` : ''}${gallery ? ` (gallery: ${gallery})` : ''} on ${finalUrl}`);
    }
  }
}
